 ...
 */
public class NewEditLanguageActivity extends NewEditItemActivity implements IconPicker.Controller{

    private static final String TAG_ICON_PICKER = "NewEditLanguageActivity::Tag::IconPicker";
    private static final String TAG_LANGUAGE_PICKER = "NewEditLanguageActivity::Tag::LanguagePicker";
    ...
    protected void onViewCreated(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            if (getMode() == Mode.EDIT_ITEM) {
                ...
                Uri uri = ContentUris.withAppendedId(DataContentProvider.CONTENT_LANGUAGES, getItemId());
                String[] projection = new String[] {
                        Contract.Language.ID,
                        Contract.Language.NAME,
                };
                Cursor cursor = contentResolver.query(uri, projection, null, null, null);
                if (cursor != null) {
                    if (cursor.moveToFirst()) {
                        long id = cursor.getLong(cursor.getColumnIndex(Contract.Language.ID));
                        String name = cursor.getString(cursor.getColumnIndex(Contract.Language.NAME));
                    }
                    cursor.close();
                }
            }
    ...
    protected int getActivityTileRes(Mode mode) {
        switch (mode) {
            case NEW_ITEM:
                return R.string.title_activity_new_place;       //ALLAGH H DIAGRAFH
            case EDIT_ITEM:
    ...
    protected void onSaveChanges(Mode mode) {
        if (mNameEditText.validate()) {
            ...
            contentValues.put(Contract.Language.NAME, mNameEditText.getTextAsString());
            switch (mode) {
                case NEW_ITEM:
                    contentResolver.insert(DataContentProvider.CONTENT_LANGUAGES, contentValues);
                    break;
                case EDIT_ITEM:
                    Uri uri = ContentUris.withAppendedId(DataContentProvider.CONTENT_LANGUAGES, getItemId());
